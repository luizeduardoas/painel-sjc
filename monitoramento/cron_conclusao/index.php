...
GSecurity::verificarPermissao("CRON_CONCLUSAO");

$breadcrumb = new Breadcrumb();
$breadcrumb->add("Início", URL_SYS, 0);
$breadcrumb->add("Monitoramento >> Logs da CRON de atualização de conclusões", $_SERVER["REQUEST_URI"], 1);

$header = new GHeader("Monitoramento >> Logs da CRON de atualização de conclusões", true);
$header->addMenu("CRON_CONCLUSAO", "Visualização de Logs da CRON de atualização de conclusões", "Visualize as informações de Logs da CRON de atualização de conclusões do sistema");
...
$html .= '    <h3 class="header smaller lighter green center">Visualização de Logs da CRON de atualização de conclusões</h3>';
$html .= '    <p>Acompanhe os logs gerados na execução do CRON automatizado para de atualização de conclusões.</p>';
...
<script type="text/javascript">
    ...
    function atualizar() {
        jQuery.gAjax.load("<?php echo URL_SYS . 'monitoramento/cron_conclusao/'; ?>load.php", {filtro_data: jQuery("#filtro_data").val()}, "#load");
    }
</script>
