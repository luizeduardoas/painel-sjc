...
GSecurity::verificarPermissao("CRON_CURSO");

$breadcrumb = new Breadcrumb();
$breadcrumb->add("Início", URL_SYS, 0);
$breadcrumb->add("Monitoramento >> Logs da CRON de atualização de curso", $_SERVER["REQUEST_URI"], 1);

$header = new GHeader("Monitoramento >> Logs da CRON de atualização de curso", true);
$header->addMenu("CRON_CURSO", "Visualização de Logs da CRON de atualização de curso", "Visualize as informações de Logs da CRON de atualização de curso do sistema");
...
$html .= '    <h3 class="header smaller lighter green center">Visualização de Logs da CRON de atualização de curso</h3>';
$html .= '    <p>Acompanhe os logs gerados na execução do CRON automatizado para de atualização de curso.</p>';
...
<script type="text/javascript">
    ...
    function atualizar() {
        jQuery.gAjax.load("<?php echo URL_SYS . 'monitoramento/cron_curso/'; ?>load.php", {filtro_data: jQuery("#filtro_data").val()}, "#load");
    }
</script>
