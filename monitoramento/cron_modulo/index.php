...
GSecurity::verificarPermissao("CRON_MODULO");

$breadcrumb = new Breadcrumb();
$breadcrumb->add("Início", URL_SYS, 0);
$breadcrumb->add("Monitoramento >> Logs da CRON de atualização de módulos", $_SERVER["REQUEST_URI"], 1);

$header = new GHeader("Monitoramento >> Logs da CRON de atualização de módulos", true);
$header->addMenu("CRON_MODULO", "Visualização de Logs da CRON de atualização de módulos", "Visualize as informações de Logs da CRON de atualização de módulos do sistema");
...
$html .= '    <h3 class="header smaller lighter green center">Visualização de Logs da CRON de atualização de módulos</h3>';
$html .= '    <p>Acompanhe os logs gerados na execução do CRON automatizado para de atualização de módulos.</p>';
...
<script type="text/javascript">
    ...
    function atualizar() {
        jQuery.gAjax.load("<?php echo URL_SYS . 'monitoramento/cron_modulo/'; ?>load.php", {filtro_data: jQuery("#filtro_data").val()}, "#load");
    }
</script>
