...
GSecurity::verificarPermissao("CRON_MATRICULA");

$breadcrumb = new Breadcrumb();
$breadcrumb->add("Início", URL_SYS, 0);
$breadcrumb->add("Monitoramento >> Logs da CRON de atualização de matrículas", $_SERVER["REQUEST_URI"], 1);

$header = new GHeader("Monitoramento >> Logs da CRON de atualização de matrículas", true);
$header->addMenu("CRON_MATRICULA", "Visualização de Logs da CRON de atualização de matrículas", "Visualize as informações de Logs da CRON de atualização de matrículas do sistema");
...
$html .= '    <h3 class="header smaller lighter green center">Visualização de Logs da CRON de atualização de matrículas</h3>';
$html .= '    <p>Acompanhe os logs gerados na execução do CRON automatizado para de atualização de matrículas.</p>';
...
<script type="text/javascript">
    ...
    function atualizar() {
        jQuery.gAjax.load("<?php echo URL_SYS . 'monitoramento/cron_matricula/'; ?>load.php", {filtro_data: jQuery("#filtro_data").val()}, "#load");
    }
</script>
