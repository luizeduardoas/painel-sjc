...
GSecurity::verificarPermissao("CRON_ACESSO");

$breadcrumb = new Breadcrumb();
$breadcrumb->add("Início", URL_SYS, 0);
$breadcrumb->add("Monitoramento >> Logs da CRON de atualização de acessos", $_SERVER["REQUEST_URI"], 1);

$header = new GHeader("Monitoramento >> Logs da CRON de atualização de acessos", true);
$header->addMenu("CRON_ACESSO", "Visualização de Logs da CRON de atualização de acessos", "Visualize as informações de Logs da CRON de atualização de acessos do sistema");
...
$html .= '    <h3 class="header smaller lighter green center">Visualização de Logs da CRON de atualização de acessos</h3>';
$html .= '    <p>Acompanhe os logs gerados na execução do CRON automatizado para de atualização de acessos.</p>';
...
<script type="text/javascript">
    ...
    function atualizar() {
        jQuery.gAjax.load("<?php echo URL_SYS . 'monitoramento/cron_acesso/'; ?>load.php", {filtro_data: jQuery("#filtro_data").val()}, "#load");
    }
</script>
