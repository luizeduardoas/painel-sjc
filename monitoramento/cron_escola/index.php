...
GSecurity::verificarPermissao("CRON_ESCOLA");

$breadcrumb = new Breadcrumb();
$breadcrumb->add("Início", URL_SYS, 0);
$breadcrumb->add("Monitoramento >> Logs da CRON de atualização de escolas", $_SERVER["REQUEST_URI"], 1);

$header = new GHeader("Monitoramento >> Logs da CRON de atualização de escolas", true);
$header->addMenu("CRON_ESCOLA", "Visualização de Logs da CRON de atualização de escolas", "Visualize as informações de Logs da CRON de atualização de escolas do sistema");
...
$html .= '    <h3 class="header smaller lighter green center">Visualização de Logs da CRON de atualização de escolas</h3>';
$html .= '    <p>Acompanhe os logs gerados na execução do CRON automatizado para de atualização de escolas.</p>';
...
<script type="text/javascript">
    ...
    function atualizar() {
        jQuery.gAjax.load("<?php echo URL_SYS . 'monitoramento/cron_escola/'; ?>load.php", {filtro_data: jQuery("#filtro_data").val()}, "#load");
    }
</script>
