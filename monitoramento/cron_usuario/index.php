...
GSecurity::verificarPermissao("CRON_USUARIO");

$breadcrumb = new Breadcrumb();
$breadcrumb->add("Início", URL_SYS, 0);
$breadcrumb->add("Monitoramento >> Logs da CRON de atualização de usuários", $_SERVER["REQUEST_URI"], 1);

$header = new GHeader("Monitoramento >> Logs da CRON de atualização de usuários", true);
$header->addMenu("CRON_USUARIO", "Visualização de Logs da CRON de atualização de usuários", "Visualize as informações de Logs da CRON de atualização de usuários do sistema");
...
$html .= '    <h3 class="header smaller lighter green center">Visualização de Logs da CRON de atualização de usuários</h3>';
$html .= '    <p>Acompanhe os logs gerados na execução do CRON automatizado para de atualização de usuários.</p>';
...
<script type="text/javascript">
    ...
    function atualizar() {
        jQuery.gAjax.load("<?php echo URL_SYS . 'monitoramento/cron_usuario/'; ?>load.php", {filtro_data: jQuery("#filtro_data").val()}, "#load");
    }
</script>
