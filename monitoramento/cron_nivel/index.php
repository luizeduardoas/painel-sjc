...
GSecurity::verificarPermissao("CRON_NIVEL");

$breadcrumb = new Breadcrumb();
$breadcrumb->add("Início", URL_SYS, 0);
$breadcrumb->add("Monitoramento >> Logs da CRON de atualização de estrutura organizacional", $_SERVER["REQUEST_URI"], 1);

$header = new GHeader("Monitoramento >> Logs da CRON de atualização de estrutura organizacional", true);
$header->addMenu("CRON_NIVEL", "Visualização de Logs da CRON de atualização de estrutura organizacional", "Visualize as informações de Logs da CRON de atualização de estrutura organizacional do sistema");
...
$html .= '    <h3 class="header smaller lighter green center">Visualização de Logs da CRON de atualização de estrutura organizacional</h3>';
$html .= '    <p>Acompanhe os logs gerados na execução do CRON automatizado para de atualização de estrutura organizacional.</p>';
...
<script type="text/javascript">
    ...
    function atualizar() {
        jQuery.gAjax.load("<?php echo URL_SYS . 'monitoramento/cron_nivel/'; ?>load.php", {filtro_data: jQuery("#filtro_data").val()}, "#load");
    }
</script>
