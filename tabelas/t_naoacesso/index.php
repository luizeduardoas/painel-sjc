...
GSecurity::verificarPermissao("T_NAOACESSO");

$breadcrumb = new Breadcrumb();
$breadcrumb->add("Início", URL_SYS . 'home/', 0);
$breadcrumb->add("Tabelas >> Não Acessaram", URL_SYS . 'tabelas/t_naoacesso/', 1);

$header = new GHeader("Tabelas dos que não acessaram", true);
$header->addMenu("T_NAOACESSO", "Tabelas dos que não acessaram", "Visualize a tabelas dos que não acessaram do sistema");
...
$html .= $form->open("form");
$html .= '<fieldset>';
global $__arrayAgrupamentoNaoAcesso;
$html .= '<div class="col-lg-3 col-xs-12 no-padding-left">';
$html .= $form->addSelect("filtro_agrupamento", $__arrayAgrupamentoNaoAcesso, buscarCookie("filtro_agrupamento", "-1"), "Agrupar", array("class" => "form-control selects chosen-select", "validate" => "([~] != -1)|Obrigatório"), array("class" => "required"));
...
$html .= '</div>';
global $__arrayOrdemNaoAcesso;
$html .= '<div class="col-lg-4 col-xs-12 no-padding-left">';
$html .= $form->addSelect("ordenacao", $__arrayOrdemNaoAcesso, buscarCookie("ordenacao", "QD"), "Ordenação", array("class" => "form-control selects", "validate" => "([~] != -1)|Obrigatório"), array("class" => "required"));
$html .= '<div class="space space-8"></div>';
$html .= '</div>';
$html .= '</fieldset>';
$html .= carregarBotoes("G");
$html .= $form->close();
$html .= gerarRodape(array('tipo' => 'box', 'col' => 10));
$html .= gerarCabecalho(array('tipo' => 'box', 'col' => 10, 'fa' => 'table', 'titulo' => 'Tabelas de não acessaram'));
...
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        jQuery("#btn_excel").click(function () {
            window.open("<?php echo URL_SYS . 'tabelas/t_naoacesso/'; ?>excel.php");
        });
        loadNivel();
    ...
    function carregarTabela(tipo, codigo) {
        jQuery.gAjax.load("<?php echo URL_SYS . 'tabelas/t_naoacesso/'; ?>load.php", {filtro_curso: jQuery("#cur_int_codigo").val(), filtro_agrupamento: jQuery("#filtro_agrupamento").val(), filtro_periodo: jQuery("#filtro_periodo").val(), ordenacao: jQuery("#ordenacao").val(), tipo: tipo, codigo: codigo}, "#div_load");
    }
</script>