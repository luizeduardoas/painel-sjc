...
GSecurity::verificarPermissao("T_MATRICULAS");

$breadcrumb = new Breadcrumb();
$breadcrumb->add("Início", URL_SYS . 'home/', 0);
$breadcrumb->add("Tabelas >> Matrículas", URL_SYS . 'tabelas/t_matriculas/', 1);

$header = new GHeader("Tabelas de Matrículas", true);
$header->addMenu("T_MATRICULAS", "Tabelas de Matrículas", "Visualize os tabelas de Matrículas do sistema");
...
$html .= $form->open("form");
$html .= '<fieldset>';
global $__arrayAgrupamentoMatriculas;
$html .= '<div class="col-lg-3 col-xs-12 no-padding-left">';
$html .= $form->addSelect("filtro_agrupamento", $__arrayAgrupamentoMatriculas, buscarCookie("filtro_agrupamento", "-1"), "Agrupar", array("class" => "form-control selects chosen-select", "validate" => "([~] != -1)|Obrigatório"), array("class" => "required"));
...
$html .= '</div>';
global $__arrayOrdemMatriculas;
$html .= '<div class="col-lg-4 col-xs-12 no-padding-left">';
$html .= $form->addSelect("ordenacao", $__arrayOrdemMatriculas, buscarCookie("ordenacao", "QD"), "Ordenação", array("class" => "form-control selects", "validate" => "([~] != -1)|Obrigatório"), array("class" => "required"));
$html .= '<div class="space space-8"></div>';
$html .= '</div>';
$html .= '</fieldset>';
...
$html .= gerarCabecalho(array('tipo' => 'box', 'col' => 6, 'fa' => 'table', 'titulo' => 'Tabelas de Matrículas'));
...
<script type="text/javascript">
    jQuery(document).ready(function () {
        ...
        jQuery("#btn_excel").click(function () {
            window.open("<?php echo URL_SYS . 'tabelas/t_matriculas/'; ?>excel.php");
        });
        loadNivel();
    ...
    function carregarTabela(tipo, codigo) {
        jQuery.gAjax.load("<?php echo URL_SYS . 'tabelas/t_matriculas/'; ?>load.php", {filtro_curso: jQuery("#cur_int_codigo").val(), filtro_tipo: jQuery("#filtro_tipo").val(), filtro_agrupamento: jQuery("#filtro_agrupamento").val(), filtro_periodo: jQuery("#filtro_periodo").val(), ordenacao: jQuery("#ordenacao").val(), tipo: tipo, codigo: codigo}, "#div_load");
    }
</script>