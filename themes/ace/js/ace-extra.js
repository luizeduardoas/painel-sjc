!function ()
{
    "ace" in window || (window.ace = {}), ace.config = {
        ...
        {
            if (!a || "string" == typeof a && !(a = document.getElementById(a)) || !a.hasAttribute("id"))
                return !1;
            if (!ace.hasClass(a, "ace-save-state"))
                return !1;
            var b = b || "class",
                    e = a.getAttribute("id"),
        ...
        loadState: function (a, b)
        {
            ...
            var f = function (a, b, c)
            {
                ...
                if (d.test(b))
                {
                    ...
                    e.test(b) ? a[b] = c : a.setAttribute(b, c)
            };
            if (void 0 !== b)
                d.hasOwnProperty(b) && null !== d[b] && f(a, b, d[b]);
            else
                for (var g in d)
        ...
        clearState: function (a)
        {
            ...
            function ()
            {
                var a = function ()
                {
                    ...
                    if (void 0 !== f.style.animationName && (a = !0), a === !1)
            ...
            }(), ace.data_storage = function (a, b)
    {
        ...
        (1 == a || a === b || 0 == a) && "localStorage" in window && null !== window.localStorage ? (d = ace.storage, e = 1) : null == d && (2 == a || a === b) && "cookie" in document && null !== document.cookie && (d = ace.cookie, e = 2), this.set = function (a, b, f, g, h, i)
        {
            if (d)
                if (f === i)
                    f = b, b = a, null == f ? d.remove(c + b) : 1 == e ? d.set(c + b, f) : 2 == e && d.set(c + b, f, ace.config.cookie_expiry, g || ace.config.cookie_path);
                else if (1 == e)
                    null == f ? d.remove(c + a + "_" + b) : (h && "object" == typeof f && (f = JSON.stringify(f)), d.set(c + a + "_" + b, f));
                else if (2 == e)
                {
                    var j = d.get(c + a),
                            k = j ? JSON.parse(j) :
                    ...
                    if (null == f)
                    {
                        if (delete k[b], 0 == ace.sizeof(k))
                            return void d.remove(c + a)
                    } else
                        k[b] = f;
