...
class GDbMysqlMoodle {
    ...
    private function connect() {
        if (!$this->link) {
            $this->link = mysqli_connect(MYSQL_MOODLE_HOST, MYSQL_MOODLE_USER, MYSQL_MOODLE_PASS, MYSQL_MOODLE_BASE);
            if (!$this->link) {
                throw new GDbException(mysqli_error($this->link), mysqli_errno($this->link));
            }
        }
        mysqli_set_charset($this->link, MYSQL_MOODLE_CHARSET);
    }
    ...
    public function execute($query, $param = NULL, $consulta = TRUE) {
        try {
            ...
            if (mysqli_stmt_execute($this->stmt)) {
                ...
                throw new GDbException(mysqli_stmt_error($this->stmt), mysqli_stmt_errno($this->stmt), $query);
            }
        } catch (Exception $e) {
            throw new GDbException($e->getMessage(), $e->getCode(), $query);
        }
    }
     ...
     * @return array
     */
    public function executeArray($query, $param = false) {
        $array = array();
        try {
            if ($param)
                $this->execute($query, $param);
