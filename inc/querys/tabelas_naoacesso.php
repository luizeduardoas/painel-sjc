        ...
        if (!seNuloOuVazioOuMenosUm($ordenacao)) {
            try {
                ...
                if ($filtro_agrupamento == 'E' && seNuloOuVazioOuZeroOuMenosUm($arrParam["tipo"] ?? null)) {
                    ...
                    $query[] = "(SELECT COUNT(DISTINCT(t.usu_int_codigo)) FROM ava_matricula t INNER JOIN ava_usuario s ON (s.usu_int_codigo = t.usu_int_codigo) WHERE s.esc_int_codigo = e.esc_int_codigo AND t.cur_int_codigo = m.cur_int_codigo) AS TOTAL ";
